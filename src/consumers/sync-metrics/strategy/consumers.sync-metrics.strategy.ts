...
import { IntegrationProvider, IntegrationProviders } from "../../../types/types.provider";
...
export interface SyncMetricsStrategy {
  ...
  ): Promise<Omit<IntegrationDailyMetric, "id" | "integration_id" | "created_at" | "updated_at">[]>;

  getSleepSessions(
    userId: User["id"],
    { startTime, endTime }: { startTime: Date; endTime: Date },
  ): Promise<Omit<SleepSession, "id" | "integration_id" | "created_at" | "updated_at">[]>;
}
